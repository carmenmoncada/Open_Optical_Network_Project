...
class Lightpath(Signal_Information):
    ...
    def __init__(self, signal_power, path, slot):    # constructor of Lightpath
        super().__init__(signal_power, path)         # Initializing constructor of Signal Information
        self.channel = slot                          # index of the channel
...
class Line:
    ...
    def __init__(self, label, length):
        ...
        self.state = list()       # free=1 or occupied=0
        for i in range(1, NUMBER_OF_CHANNELS):
            self.state.append(1)  # Free = 1 AND occupied = 0
    ...
    def propagate_method(self, line, signal_information):      # Propagates the signal information along the line
        print('Propagating Line', line.label)                  # that signali_info implies the info of the lightpaths too
        # Generating latency
...
class Network:
            ...
            for line in self.lines:
                if line.startswith(node):
                    for i in range(len(line)):  # to avoid adding root node in the successive node dictionary
                        if i != 0:
                            # filling up the successive dict with the Lines obj
    ...
    def dataframe(self, signal_power):
            ...
            for node_destination in self.nodes:  # node destination
                if node_source != node_destination:  # Finding all possible paths starting from A-B
                    ...
                    for index1 in range(len(paths_list)):  # Iterate over each list
                        ...
                        signal_info = self.probe(signal_info)                 # Propagate method = probe method
        ...
        # Set all values in columns '1' to '10' to 1 (Initializing
    ...
    def find_best_snr(self, node_input, node_output):
        ...
        for index, row in paths_btw_nodes_df.iterrows():
            for channel in range(1,NUMBER_OF_CHANNELS+1):
                ...
                column_channel = int(idx[0])
                if (current_snr > max_snr) and (self.route_space.iloc[index, column_channel] == 1):
                    max_snr = current_snr
                    best_path = row['Paths']
        ...
        if index_channel is not -1:
            ...
            for line in lines_current_list:
                self.lines[line].state[index_channel] = 0       # free=1 or occupied=0
    ...
    def find_best_latency(self, node1, node2): # Returns a path with best latency (min latency)
        ...
        for index, row in filter_path.iterrows():
            for channel in range(1,NUMBER_OF_CHANNELS+1):
                ...
                idx = [idx for idx, col in enumerate(self.route_space.columns) if str(channel) == col]
                column_channel = int(idx[0])
                if (current_latency > max_latency) and (self.route_space.iloc[int(index), column_channel] == 1):
    ...
    def stream(self, connection_list, signal_power, label):
        ...
        for list in range(len(connection_list)):
            if (label == 'latency') or (label == 'Latency'):
                best_path_index, index_channel = self.find_best_latency(connection_list[list].input, connection_list[list].output)
                print(best_path_index)
            else:  # if (label == 'snr') or (label == 'SNR'):
                # For the SNR
                best_path_index, index_channel = self.find_best_snr(connection_list[list].input, connection_list[list].output)
            ...
            if best_path_index != -1:
                ...
                # propagation_ = self.probe(signal_propagation)
                # Modifying the latency and snr of the path for each Connection
                # connection_list[list].latency = propagation_.latency
                # if propagation_.noise_power != 0:
                #     connection_list[list].snr = 10 * log(signal_power / propagation_.noise_power)  # In dB
                lat_val = self.weighted_paths.loc[best_path_index, 'Accumulated Latency']
                snr_val = self.weighted_paths.loc[best_path_index, 'Accumulated SNR']  # SNR [dB]
                connection_list[list].latency = lat_val
                connection_list[list].snr = snr_val

                # Propagating through the lightpath the signal information
                signal_propagation = Lightpath(signal_power, resulting_path, index_channel)
                ...
                if best_path_index != -1 and index_channel != -1:
                    self.route_space.loc[best_path_index, str(index_channel)] = 0       # occupied=0
            else:
                # When there is no path available case
                connection_list[list].latency = 0
                connection_list[list].snr = None    # In dB
        print(self.route_space.to_string())
...
def main():
    ...
    axs[0].set_ylabel('Density', **csfont)
    ...
    axs[1].set_ylabel('Density', **csfont)
    plt.xticks(fontsize=10, fontname='Times New Roman')
    plt.yticks(fontsize=10, fontname='Times New Roman')
